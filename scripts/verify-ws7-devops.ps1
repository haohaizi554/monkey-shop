...
Assert-Match -Name "grafana-dashboard.yaml" -Text $grafanaDashboard -Pattern "order_total" -Message "must graph business order counters"
...
if (-not $helm) {
    ...
    foreach ($environment in @("staging", "prod")) {
        ...
        Assert-Match -Name "rendered $environment" -Text $manifest -Pattern "order_total" -Message "must render business metric dashboard panels"
