...
Assert-Match -Name "ApiRateLimitFilter.java" -Text $rateLimitFilter -Pattern "RateLimitPolicy\.LOGIN" -Message "must map login requests"
Assert-Match -Name "ApiRateLimitFilter.java" -Text $rateLimitFilter -Pattern "RateLimitPolicy\.REGISTER" -Message "must map registration requests"
Assert-Match -Name "ApiRateLimitFilter.java" -Text $rateLimitFilter -Pattern "RateLimitPolicy\.ORDER" -Message "must map order creation requests"
Assert-Match -Name "ApiRateLimitFilter.java" -Text $rateLimitFilter -Pattern "RateLimitPolicy\.SEARCH" -Message "must map product search requests"
Assert-Match -Name "ApiRateLimitFilter.java" -Text $rateLimitFilter -Pattern "RateLimitPolicy\.UPLOAD" -Message "must map upload requests"
