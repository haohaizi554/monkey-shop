...
class JpaProductCatalogTest {
    ...
    @Test
    void findAllMapsRepositoryEntitiesToDomainRecords() {
        when(monkeyRepository.findAll()).thenReturn(List.of(monkey()));

        List<ProductRecord> result = catalog.findAll();

        assertThat(result).containsExactly(record());
        verify(monkeyRepository).findAll();
    }
