...
class TraceIdFilterTest {
    ...
    @Test
    void generatesTraceIdWhenHeaderIsMissingAndClearsMdc() throws Exception {
        MockHttpServletRequest request = new MockHttpServletRequest();
        MockHttpServletResponse response = new MockHttpServletResponse();

        filter.doFilter(request, response, (servletRequest, servletResponse) -> {
            assertThat(MDC.get(TraceIds.MDC_KEY)).isNotBlank();
            assertThat(response.getHeader(TraceIds.HEADER)).isEqualTo(MDC.get(TraceIds.MDC_KEY));
        });
    ...
    @Test
    void rejectsOverlongTraceId() throws Exception {
        MockHttpServletRequest request = new MockHttpServletRequest();
        request.addHeader(TraceIds.HEADER, "a".repeat(129));
        MockHttpServletResponse response = new MockHttpServletResponse();
        ...
        filter.doFilter(
                ...
                response,
                (servletRequest, servletResponse) ->
                        assertThat(MDC.get(TraceIds.MDC_KEY)).hasSizeLessThanOrEqualTo(128));

        assertThat(response.getHeader(TraceIds.HEADER)).hasSizeLessThanOrEqualTo(128);
        assertThat(response.getHeader(TraceIds.HEADER)).isNotEqualTo("a".repeat(129));
        assertThat(MDC.get(TraceIds.MDC_KEY)).isNull();
    }
