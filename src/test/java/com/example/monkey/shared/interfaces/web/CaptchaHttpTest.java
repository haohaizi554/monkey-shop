...
class CaptchaHttpTest {
    ...
    void writesLocalChallengeCookieAndImageResponse() throws Exception {
        CaptchaChallengeResult challenge = CaptchaChallengeResult.local(
                "challenge-id", Duration.ofMinutes(5), true, "image/jpeg", new byte[] {1, 2, 3});
    ...
    void writesExternalProviderMetadataWithoutBody() throws Exception {
        CaptchaChallengeResult challenge = CaptchaChallengeResult.external("turnstile", "site-key");
        MockHttpServletResponse response = new MockHttpServletResponse();
