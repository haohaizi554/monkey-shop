...
class JpaAddressBookTest {
    ...
    @Test
    void findPagedByUserIdFiltersUnsupportedSortPropertiesBeforeQueryingJpa() {
        when(addressRepository.findByUserId(any(), any(Pageable.class)))
                .thenReturn(new PageImpl<>(List.of(address()), PageRequest.of(0, 10), 1));

        addressBook.findByUserId(
                7L,
                new AddressPageRequest(
                        0, 10, List.of(new SortOrder("phone", Direction.ASC), new SortOrder("id", Direction.DESC))));

        Pageable pageable = capturePageable();
        assertThat(pageable.getSort().getOrderFor("phone")).isNull();
        assertThat(pageable.getSort().getOrderFor("id").getDirection()).isEqualTo(Sort.Direction.DESC);
    }

    @Test
    void findPagedByUserIdBoundsPageAndSizeBeforeQueryingJpa() {
        when(addressRepository.findByUserId(any(), any(Pageable.class)))
                .thenReturn(new PageImpl<>(List.of(address()), PageRequest.of(0, 100), 1));

        addressBook.findByUserId(7L, new AddressPageRequest(-2, 5000, List.of()));
