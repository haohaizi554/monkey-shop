...
- Content-Security-Policy is intentionally generated by Spring Security so the `script-src` and `style-src` nonces can be unique per request; do not override it with a static Nginx header.
